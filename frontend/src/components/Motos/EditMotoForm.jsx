...
const EditMotoForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [motoRes, clientesRes] = await Promise.all([
          motosApi.getById(id),
          clientesApi.getAll(),
        ]);

        const { placa, marca, modelo, clientId } = motoRes.data;
        setFormData({
          placa: placa || "",
          marca: marca || "",
          modelo: modelo || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error ?? "Error al actualizar la moto.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm max-w-md mx-auto">
      <h2 className="text-lg font-bold text-gray-800 mb-4">Editar Moto</h2>
